import { registerChapter } from '../../../registry.js';

registerChapter('economie|tl|3|5', {
title:'Is er werk voor jou?',
summary:[
{heading:'5.1 Wat levert werken op?',html:
 '<div class="box"><h4>Waar dit hoofdstuk over gaat</h4><p>Straks ga jij misschien ook werken. Je leert wat werken je oplevert (bruto- en nettoloon), welke ondernemingsvormen er zijn, hoe de arbeidsmarkt werkt en wat er gebeurt als je (tijdelijk) geen werk hebt.</p></div>'

 +'<h5 class="sub">Geschoold of ongeschoold?</h5>'
 +'<div class="g2">'
 +'<div class="box"><h4>Geschoold werk</h4><p>Voor <b>geschoold werk</b> heb je een beroepsopleiding nodig.</p></div>'
 +'<div class="box"><h4>Ongeschoold werk</h4><p>Voor ongeschoold werk, zoals een bijbaantje in een supermarkt, heb je geen opleiding nodig.</p></div></div>'

 +'<h5 class="sub">Van brutoloon naar nettoloon</h5>'
 +'<div class="g2">'
 +'<div class="box"><h4>Brutoloon</h4><p>Het <b>brutoloon</b> is het loon waarop nog niets is ingehouden.</p></div>'
 +'<div class="box"><h4>Nettoloon</h4><p>Het <b>nettoloon</b> is het loon dat je uiteindelijk ontvangt, waar de inhoudingen al vanaf zijn gehaald.</p></div></div>'
 +'<div class="box"><h4>Wat houdt je werkgever in?</h4><p>Op je brutoloon houdt je werkgever loonbelasting en sociale premies in. Hij draagt deze af aan de overheid.</p></div>'

 +'<div class="call reken"><b>Reken mee: van bruto naar netto</b>'
 +'<p>Je brutoloon is € 1.850. Daarop wordt € 165 loonbelasting en € 92 sociale premies ingehouden.</p>'
 +'<ol class="num">'
 +'<li><b>Stap 1</b>. Tel de inhoudingen bij elkaar op: € 165 + € 92 = € 257.</li>'
 +'<li><b>Stap 2</b>. Trek dit af van het brutoloon: € 1.850 − € 257.</li>'
 +'<li class="eind"><b>Antwoord</b>: Nettoloon = <b>€ 1.593</b>.</li></ol></div>'

 +'<div class="box"><h4>Het minimumloon</h4><p>Iedereen vanaf 21 jaar moet minstens het <b>minimumloon</b> per uur verdienen.</p></div>'
 +'<div class="box"><h4>De cao</h4><p>De meeste afspraken in je arbeidsovereenkomst komen uit de <b>cao</b> (collectieve arbeidsovereenkomst). Hierin staan de gezamenlijke afspraken over de arbeidsvoorwaarden in een bedrijfstak, zoals de bouw, de industrie of het onderwijs. Cao-afspraken worden gemaakt door vakbonden (die opkomen voor de belangen van werknemers) en organisaties van werkgevers.</p></div>'

 +'<div class="call warn"><b>Veelgemaakte fout:</b> verwar brutoloon en nettoloon niet. Het brutoloon staat in je arbeidsovereenkomst, maar het nettoloon is het bedrag dat écht op je bankrekening binnenkomt.</div>'

 +'<div class="call sum"><b>Om te onthouden</b><ul class="lst">'
 +'<li>Nettoloon = brutoloon − (loonbelasting + sociale premies).</li>'
 +'<li>Het minimumloon geldt vanaf 21 jaar.</li>'
 +'<li>De cao bevat afspraken over arbeidsvoorwaarden in een bedrijfstak, gemaakt door vakbonden en werkgevers.</li></ul></div>'},

{heading:'5.2 Wat voor ondernemingen?',html:
 '<div class="box"><h4>Werken voor jezelf</h4><p>De meeste mensen met een betaalde baan zijn werknemer in loondienst. Maar je kunt ook als <b>zelfstandige</b> werken: iemand die met een eigen onderneming een inkomen verdient, door goederen of diensten te leveren aan klanten.</p><p>Wie een bedrijf begint, schrijft zich in bij de Kamer van Koophandel (KvK) en kiest een ondernemingsvorm.</p></div>'

 +'<h5 class="sub">De ondernemingsvormen</h5>'
 +'<div class="tblwrap"><table class="tbl"><tr><th>Vorm</th><th>Kenmerken</th></tr>'
 +'<tr><td>Eenmanszaak</td><td>Eén eigenaar. Kan personeel in dienst hebben. Bij schulden moet de eigenaar die desnoods met privégeld afbetalen.</td></tr>'
 +'<tr><td>Zzp\'er</td><td>Zelfstandige zonder personeel. Kiest meestal voor een eenmanszaak als ondernemingsvorm.</td></tr>'
 +'<tr><td>VOF</td><td>Vennootschap onder firma. Meerdere eigenaren die samen de leiding hebben.</td></tr>'
 +'<tr><td>BV</td><td>Besloten vennootschap. Eén of meer eigenaren als aandeelhouder; de aandelen zijn niet voor iedereen te koop.</td></tr>'
 +'<tr><td>NV</td><td>Naamloze vennootschap. Meerdere eigenaren als aandeelhouder; iedereen kan aandelen kopen.</td></tr></table></div>'

 +'<h5 class="sub">Elk bedrijf in z\'n eigen sector</h5>'
 +'<div class="box"><p>Bedrijven en instellingen kun je indelen in vier <b>productiesectoren</b>:</p></div>'
 +'<div class="tblwrap"><table class="tbl"><tr><th>Sector</th><th>Voorbeelden</th></tr>'
 +'<tr><td>Primaire sector</td><td>Agrarische bedrijven: landbouw, visserij, winning van grondstoffen</td></tr>'
 +'<tr><td>Secundaire sector</td><td>Industriële bedrijven: fabrieken, de bouw, ambachten (zoals een bakker)</td></tr>'
 +'<tr><td>Tertiaire sector</td><td>Commerciële dienstverlening: winkels, banken, transportbedrijven</td></tr>'
 +'<tr><td>Quartaire sector</td><td>Niet-commerciële dienstverlening: onderwijs, gezondheidszorg, overheid</td></tr></table></div>'

 +'<div class="call sum"><b>Om te onthouden</b><ul class="lst">'
 +'<li>Eenmanszaak en zzp: één eigenaar. VOF: meerdere eigenaren die samen leiden. BV/NV: eigenaren zijn aandeelhouders.</li>'
 +'<li>Vier productiesectoren: primair (grondstoffen), secundair (industrie), tertiair (commerciële dienstverlening), quartair (niet-commerciële dienstverlening).</li></ul></div>'},

{heading:'5.3 Doe jij mee op de arbeidsmarkt?',html:
 '<div class="box"><h4>De arbeidsmarkt</h4><p>Heb je een baantje, zoek je werk of heb je als ondernemer personeel nodig? Dan ben je actief op de <b>arbeidsmarkt</b>: het geheel van vraag naar arbeid en aanbod van arbeid.</p></div>'

 +'<h5 class="sub">Vraag en aanbod van arbeid</h5>'
 +'<div class="g2">'
 +'<div class="box"><h4>Vraag naar arbeid</h4><p>Komt van bedrijven en de overheid: zij vragen mensen om voor hen te komen werken. De vraag naar arbeid bepaalt de <b>werkgelegenheid</b>: alle arbeidsplaatsen bij bedrijven en de overheid. Groeit de economie, dan verkopen bedrijven meer en hebben ze meer mensen nodig: de werkgelegenheid neemt toe.</p></div>'
 +'<div class="box"><h4>Aanbod van arbeid</h4><p>Komt van de <b>beroepsbevolking</b>: iedereen van vijftien jaar tot de pensioenleeftijd die werkt of als werkloze op zoek is naar werk. De beroepsbevolking groeit bijvoorbeeld elke zomer doordat jongeren na hun opleiding een baan zoeken, en krimpt als mensen met pensioen gaan, overlijden of niet meer willen werken.</p></div></div>'

 +'<div class="call">De <b>arbeidsparticipatie</b> (arbeidsdeelname) is het percentage van de bevolking dat tot de beroepsbevolking behoort.</div>'
 +'<div class="call">De <b>Algemene wet gelijke behandeling</b> verbiedt het maken van onderscheid op basis van gender, religie, leeftijd of afkomst bij bijvoorbeeld solliciteren.</div>'
 +'<div class="call">Heb je alleen werk wanneer je werkgever je nodig heeft? Dan heb je een <b>flexibele baan</b>.</div>'

 +'<div class="call sum"><b>Om te onthouden</b><ul class="lst">'
 +'<li>Arbeidsmarkt = vraag naar arbeid (werkgelegenheid) + aanbod van arbeid (beroepsbevolking).</li>'
 +'<li>Beroepsbevolking: 15 jaar tot pensioenleeftijd, werkend of werkzoekend.</li>'
 +'<li>De Algemene wet gelijke behandeling verbiedt discriminatie op de arbeidsmarkt.</li></ul></div>'},

{heading:'5.4 Als je zonder werk zit',html:
 '<div class="box"><h4>Waarom raak je werkloos?</h4><p><b>Werkloosheid</b> ontstaat als het aanbod van arbeid groter is dan de vraag ernaar. Er zijn verschillende soorten werkloosheid, met elk een andere oorzaak:</p></div>'

 +'<div class="tblwrap"><table class="tbl"><tr><th>Soort werkloosheid</th><th>Oorzaak</th></tr>'
 +'<tr><td>Frictiewerkloosheid</td><td>Kortdurende werkloosheid omdat je tijd nodig hebt om een nieuwe baan te vinden</td></tr>'
 +'<tr><td>Conjuncturele werkloosheid</td><td>Gevolg van een daling van de vraag naar goederen en diensten door vermindering van koopkracht</td></tr>'
 +'<tr><td>Structurele werkloosheid</td><td>Gevolg van veranderingen in de aanbodkant van de economie, bijvoorbeeld automatisering</td></tr>'
 +'<tr><td>Seizoenwerkloosheid</td><td>Werk kan alleen in een deel van het jaar gedaan worden</td></tr>'
 +'<tr><td>Regionale werkloosheid</td><td>Werkloosheid die in bepaalde gebieden hoger is dan gemiddeld in het land</td></tr></table></div>'

 +'<div class="call">Het <b>UWV</b> is de overheidsinstelling die je helpt bij het zoeken naar een nieuwe baan en die beoordeelt of je recht hebt op een WW-uitkering.</div>'

 +'<div class="call warn"><b>Veelgemaakte fout:</b> gooi de soorten werkloosheid niet op één hoop. Kijk steeds naar de oorzaak: ligt het aan de economie als geheel (conjunctureel), aan blijvende veranderingen (structureel), aan het seizoen, aan de regio of gewoon aan de tijd die zoeken kost (frictie)?</div>'

 +'<div class="call sum"><b>Om te onthouden</b><ul class="lst">'
 +'<li>Werkloosheid = aanbod van arbeid groter dan de vraag.</li>'
 +'<li>Vijf soorten: frictie-, conjuncturele, structurele, seizoen- en regionale werkloosheid.</li>'
 +'<li>Het UWV helpt bij het zoeken naar werk en beoordeelt het recht op een WW-uitkering.</li></ul></div>'}
],
terms:[
['Brutoloon','Het loon waarop nog niets is ingehouden.',0],
['Nettoloon','Het loon dat je ontvangt en waar de inhoudingen al vanaf gehaald zijn.',0],
['Minimumloon','Het loon dat je vanaf 21 jaar minstens per uur moet verdienen.',0],
['Cao','Collectieve arbeidsovereenkomst. Hierin staan de gezamenlijke afspraken over de arbeidsvoorwaarden in een bedrijfstak.',0],
['Geschoold werk','Werk waarvoor je een beroepsopleiding nodig hebt.',0],
['Zwartwerk','Betaald werk waarover je geen belasting of sociale premies betaalt.',0],
['Zelfstandige','Iemand die met een eigen onderneming een inkomen verdient.',1],
['Zzp\'er','Zelfstandige zonder personeel.',1],
['Eenmanszaak','Onderneming met één eigenaar.',1],
['VOF','Vennootschap onder firma. Een onderneming met meerdere eigenaren die samen de leiding hebben.',1],
['BV','Besloten vennootschap. Onderneming met één of meer eigenaren als aandeelhouder; de aandelen zijn niet voor iedereen te koop.',1],
['NV','Naamloze vennootschap. Onderneming met meerdere eigenaren die aandeelhouder zijn, waarvan iedereen aandelen kan kopen.',1],
['Productiesectoren','Indeling van productie en arbeid in primaire, secundaire, tertiaire en quartaire sector.',1],
['Arbeidsverdeling','Het werk bij bedrijven is verdeeld in verschillende banen.',1],
['Arbeidsmarkt','Het geheel van vraag naar arbeid en aanbod van arbeid.',2],
['Werkgelegenheid','Alle arbeidsplaatsen bij bedrijven en de overheid.',2],
['Beroepsbevolking','Iedereen van vijftien jaar tot de pensioenleeftijd die werkt of werkloos is.',2],
['Arbeidsparticipatie','Arbeidsdeelname: het percentage van de bevolking dat tot de beroepsbevolking behoort.',2],
['Algemene wet gelijke behandeling','Wet die het maken van onderscheid op basis van gender, religie, leeftijd of afkomst verbiedt.',2],
['Flexibele baan','Je hebt alleen werk wanneer een bedrijf je nodig heeft.',2],
['Werkloosheid','Het aanbod van arbeid is groter dan de vraag ernaar.',3],
['Frictiewerkloosheid','Kortdurende werkloosheid omdat je tijd nodig hebt om een nieuwe baan te vinden.',3],
['Conjuncturele werkloosheid','Werkloosheid als gevolg van een daling van de vraag naar goederen en diensten door vermindering van koopkracht.',3],
['Structurele werkloosheid','Werkloosheid als gevolg van veranderingen in de aanbodkant van de economie.',3],
['Seizoenwerkloosheid','Werkloosheid doordat werk alleen in een deel van het jaar gedaan kan worden.',3],
['Regionale werkloosheid','Werkloosheid die in bepaalde gebieden hoger is dan gemiddeld in het land.',3],
['UWV','Overheidsinstelling die helpt bij het zoeken naar een nieuwe baan en die beoordeelt of je recht hebt op een WW-uitkering.',3]
],
cards:[
['Wat is het verschil tussen brutoloon en nettoloon?','Brutoloon is het loon vóór inhoudingen; nettoloon is wat je overhoudt nadat loonbelasting en sociale premies zijn ingehouden.',0],
['Vanaf welke leeftijd geldt het minimumloon?','Vanaf 21 jaar.',0],
['Wat staat er in een cao?','De gezamenlijke afspraken over arbeidsvoorwaarden in een bedrijfstak.',0],
['Wat is zwartwerk?','Betaald werk waarover je geen belasting of sociale premies betaalt.',0],
['Wat is het verschil tussen een eenmanszaak en een zzp\'er?','Een eenmanszaak kan personeel hebben; een zzp\'er is een zelfstandige zonder personeel (en kiest meestal ook voor een eenmanszaak als vorm).',1],
['Wat is een VOF?','Vennootschap onder firma: een onderneming met meerdere eigenaren die samen de leiding hebben.',1],
['Wat is het verschil tussen een BV en een NV?','Bij een BV zijn de aandelen niet voor iedereen te koop; bij een NV kan iedereen aandelen kopen.',1],
['Noem de vier productiesectoren.','Primaire, secundaire, tertiaire en quartaire sector.',1],
['Wat is de arbeidsmarkt?','Het geheel van vraag naar arbeid en aanbod van arbeid.',2],
['Wie vormen de beroepsbevolking?','Iedereen van vijftien jaar tot de pensioenleeftijd die werkt of werkloos is.',2],
['Wat bepaalt de werkgelegenheid?','De vraag naar arbeid vanuit bedrijven en de overheid.',2],
['Wat verbiedt de Algemene wet gelijke behandeling?','Onderscheid maken op basis van gender, religie, leeftijd of afkomst.',2],
['Wat is frictiewerkloosheid?','Kortdurende werkloosheid omdat je tijd nodig hebt om een nieuwe baan te vinden.',3],
['Wat veroorzaakt conjuncturele werkloosheid?','Een daling van de vraag naar goederen en diensten door verminderde koopkracht.',3],
['Wat is structurele werkloosheid?','Werkloosheid door blijvende veranderingen in de aanbodkant van de economie, zoals automatisering.',3],
['Wat doet het UWV?','Het helpt bij het zoeken naar een nieuwe baan en beoordeelt of je recht hebt op een WW-uitkering.',3]
],
quiz:[
['Je brutoloon is € 1.850. Er wordt € 165 loonbelasting en € 92 sociale premies ingehouden. Wat is je nettoloon?',['€ 1.593','€ 1.685','€ 1.758','€ 2.107'],0,'€ 1.850 − (€ 165 + € 92) = € 1.593.'],
['Vanaf welke leeftijd heb je recht op het volledige minimumloon?',['16 jaar','18 jaar','21 jaar','25 jaar'],2,'Het minimumloon geldt vanaf 21 jaar.'],
['Wat staat er in een cao?',['Je persoonlijke belastingaangifte','Gezamenlijke afspraken over arbeidsvoorwaarden in een bedrijfstak','De regels van de Kamer van Koophandel','De hoogte van alle uitkeringen'],1,'Een cao (collectieve arbeidsovereenkomst) bevat afspraken over arbeidsvoorwaarden in een bedrijfstak.'],
['Wat is het verschil tussen een eenmanszaak en een VOF?',['Een eenmanszaak heeft één eigenaar, een VOF heeft meerdere eigenaren','Een VOF mag geen personeel hebben','Een eenmanszaak kan geen schulden hebben','Er is geen verschil'],0,'Een eenmanszaak heeft één eigenaar; een VOF heeft meerdere eigenaren die samen de leiding hebben.'],
['Bij welke ondernemingsvorm zijn de eigenaren aandeelhouders van wie iedereen aandelen kan kopen?',['Eenmanszaak','VOF','BV','NV'],3,'Bij een NV (naamloze vennootschap) kan iedereen aandelen kopen; bij een BV zijn de aandelen niet voor iedereen te koop.'],
['Een supermarkt hoort bij welke productiesector?',['Primaire sector','Secundaire sector','Tertiaire sector','Quartaire sector'],2,'Een supermarkt is een commercieel dienstverlenend bedrijf: tertiaire sector.'],
['Een school hoort bij welke productiesector?',['Primaire sector','Secundaire sector','Tertiaire sector','Quartaire sector'],3,'Onderwijs is niet-commerciële dienstverlening: quartaire sector.'],
['Wat is de arbeidsmarkt?',['De plek waar je je cao afsluit','Het geheel van vraag naar en aanbod van arbeid','Een overzicht van alle vacatures','Het percentage werklozen'],1,'De arbeidsmarkt is het geheel van vraag naar arbeid (werkgelegenheid) en aanbod van arbeid (beroepsbevolking).'],
['Wie hoort tot de beroepsbevolking?',['Alleen mensen met een baan','Iedereen van 15 jaar tot de pensioenleeftijd die werkt of werkloos is','Alle inwoners van Nederland','Alleen mensen ouder dan 21 jaar'],1,'De beroepsbevolking bestaat uit iedereen van 15 jaar tot de pensioenleeftijd die werkt of werkloos is.'],
['Wat bepaalt vooral de werkgelegenheid?',['De vraag naar arbeid vanuit bedrijven en de overheid','Het aanbod van arbeid','De hoogte van het minimumloon','De inflatie'],0,'De vraag naar arbeid vanuit werkgevers bepaalt de werkgelegenheid.'],
['Door automatisering verdwijnen blijvend banen in een bepaalde sector. Dit heet...',['frictiewerkloosheid','conjuncturele werkloosheid','structurele werkloosheid','seizoenwerkloosheid'],2,'Structurele werkloosheid ontstaat door blijvende veranderingen in de aanbodkant van de economie, zoals automatisering.'],
['Werkloosheid in de horeca in de wintermaanden aan de kust is een voorbeeld van...',['structurele werkloosheid','seizoenwerkloosheid','conjuncturele werkloosheid','regionale werkloosheid'],1,'Seizoenwerkloosheid ontstaat doordat werk alleen in een deel van het jaar gedaan kan worden.'],
['Wat doet het UWV?',['Het betaalt iedereen het minimumloon','Het helpt bij het zoeken naar werk en beoordeelt WW-recht','Het maakt cao-afspraken','Het int de inkomstenbelasting'],1,'Het UWV helpt werkzoekenden en beoordeelt of iemand recht heeft op een WW-uitkering.']
]
});
